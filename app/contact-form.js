  ...
  return (
    <div className="diagnosticShell">
      <div className="diagnosticTopbar">
        ...
        <div className="diagnosticProgressCopy"><span>Pergunta {step + 1} de {totalSteps}</span><strong>{progress}%</strong></div>
      </div>
      <div className="diagnosticProgress" aria-hidden="true"><span style={{ width: `${progress}%` }} /></div>
      <div className="diagnosticDots" aria-hidden="true">{Array.from({ length: totalSteps }).map((_, index) => <i className={index <= step ? 'active' : ''} key={index} />)}</div>
      {!isFinalStep ? (
        <div className="diagnosticQuestion">
          <span className="kicker">{steps[step].eyebrow}</span><h3>{steps[step].title}</h3><p>{steps[step].description}</p>
          <div className="diagnosticOptions">{steps[step].options.map(([label, icon, description]) => <button type="button" className="diagnosticOption" key={label} onClick={() => choose(steps[step].key, label)}><span className="diagnosticIcon">{icon}</span><span><strong>{label}</strong><small>{description}</small></span><b>→</b></button>)}</div>
        </div>
      ) : (
        <form className="contactForm diagnosticForm" onSubmit={onSubmit}>
          <div className="diagnosticQuestion finalQuestion"><span className="kicker">6. CONTATO</span><h3>Quase lá. Agora precisamos falar com você.</h3><p>Os dados abaixo serão usados somente para responder sobre este projeto.</p></div>
          <input className="hp" name="website" tabIndex="-1" autoComplete="off" aria-hidden="true" />
          <div className="formRow"><label>Nome completo<input name="name" required maxLength="100" placeholder="Seu nome" /></label><label>E-mail<input name="email" type="email" required maxLength="160" placeholder="voce@empresa.com" /></label></div>
          <div className="formRow"><label>Telefone<input name="phone" maxLength="40" placeholder="(00) 00000-0000" /></label><label>Empresa<input name="company" maxLength="140" placeholder="Nome da empresa" /></label></div>
          <div className="formRow"><label>Tipo de projeto<select name="projectType" defaultValue={answers.projectType || ''}><option value="" disabled>Selecione</option><option>Software sob medida</option><option>MVP / Produto digital</option><option>IA / Automação</option><option>Evolução / Sustentação</option><option>Integração / API</option><option>Modernização</option><option>Outro</option></select></label><label>Número de colaboradores<select name="employees" defaultValue=""><option value="">Opcional</option><option>1–10</option><option>11–50</option><option>51–100</option><option>Acima de 100</option></select></label></div>
          <label>Prazo desejado<select name="deadline" defaultValue={answers.priority || ''}><option value="">Opcional</option><option>Imediatamente</option><option>Em 1 a 3 meses</option><option>Em 3 a 6 meses</option><option>Estou pesquisando</option></select></label>
          <label>Conte mais sobre seu projeto<textarea name="details" maxLength="2200" rows="5" placeholder="Objetivo, cenário atual, principais dificuldades ou qualquer informação que ajude a entender o contexto." /></label>
          <button className="button diagnosticSubmit" type="submit" disabled={status === 'sending'}>{status === 'sending' ? 'Enviando...' : 'Enviar diagnóstico →'}</button>
