...
Variáveis:
  TEST_BASE_URL       URL base obrigatória para homologação/produção
  TEST_PATHS          GETs separados por vírgula (padrão: /)
...
BASE = os.getenv("TEST_BASE_URL", "").rstrip("/")
PATHS = [p.strip() for p in os.getenv("TEST_PATHS", "/").split(",") if p.strip()]
...
def main() -> int:
    if not BASE:
        print("❌ Defina TEST_BASE_URL com a URL de homologação ou produção.")
        return 2
